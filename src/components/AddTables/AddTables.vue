		...
		<template v-for="item in columns" :key="item.dataIndex">
			<a-table-column :title="item.title" width="70px" align="center" :fixed="item.fixed">
				<template #default="{ record }">
					<span v-if="item.type === 'text'">{{ record[item.dataIndex] }}</span>
					<a-input v-if="item.type === 'input'" v-model:value="record[item.dataIndex]" size="small" :placeholder="item.placeholder" />
					<a-button v-if="item.type === 'button'" @click="staticDelect(record)" size="small" type="primary" danger>
